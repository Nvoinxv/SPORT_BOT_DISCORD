...
class GeminiService:
    ...
    async def get_event_news_summary(
        ...
        if home_score is not None and away_score is not None:
            score_info = (
                f"Skor akhir: {home_team} {home_score} - "
    ...
    async def generate_raw(self, prompt: str) -> str:
        """Generate teks bebas dari prompt apa pun. Untuk support kategori baru."""
        if not self.is_ready:
    ...
    async def generate_sport_shoes_angle(
        ...
        score_info = (
            f"Skor: {home_team} {home_score or '-'} - "
            f"{away_score or '-'} {away_team}"
            if home_score and away_score
            else "Skor belum tersedia."
        )
