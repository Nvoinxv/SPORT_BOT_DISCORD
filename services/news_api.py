...
NEWSDATA_API_KEY = os.getenv("NEWSDATA_API_KEY", "") or os.getenv("News_Data_Io_API", "")
...
class GNewsArticle:
    """Representasi artikel dari response GNews API."""
    ...
    @classmethod
    def from_raw(cls, raw: dict[str, Any]) -> "GNewsArticle":
        return cls(
...
class GNewsClient:
    """
    Client tipis di atas GNews.io REST API.
    """
    ...
    def search(
        ...
        if not self._api_key:
            logger.warning("GNEWS_API_KEY tidak tersedia, skip pencarian berita GNews.")
        ...
        return [GNewsArticle.from_raw(a) for a in articles]

    def search_sneakers(self, brand: str = "sneakers", max_results: int = 5) -> list[GNewsArticle]:
        clean_brand = brand.replace(" dan ", " ")
        query = f"{clean_brand} sneakers"
        return self.search(query=query, max_results=max_results, language="en")
